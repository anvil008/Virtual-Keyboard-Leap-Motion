...
import io.github.anvil008.keyboard.motion.GestureControl;
...
import io.github.anvil008.keyboard.robot.FXRobotHandler;
import io.github.anvil008.keyboard.robot.IRobot;
import io.github.anvil008.keyboard.xml.KeyboardLayoutHandler;
...
public class KeyboardPane extends Region implements StandardKeyCode, EventHandler<KeyButtonEvent> {
	...
	private final String DEFAULT_CSS = "/css/KeyboardTheme.css";
	...
	private final ObjectProperty<DefaultLayer> layerProperty = new SimpleObjectProperty<>(DefaultLayer.KEYBOARDPANE);
    ...
    final GestureControl mouse=new GestureControl();
	...
	private void setLayoutLocale(final Locale local) throws MalformedURLException, IOException, URISyntaxException {
		...
		addTypeRegion(KeyboardType.TEXT, root, "keyboardlayout.xml");
		addTypeRegion(KeyboardType.TEXT_SHIFT, root, "keyboardlayoutshift.xml");
		addTypeRegion(KeyboardType.SYMBOL, root, "keyboardlayoutsym.xml");
		addTypeRegion(KeyboardType.SYMBOL_SHIFT, root, "keyboardlayoutsymshift.xml");
		addTypeRegion(KeyboardType.CTRL, root, "keyboardlayoutctrl.xml");
		addTypeRegion(KeyboardType.NUMERIC, root, "keyboardlayoutnumeric.xml");
	...
	public void handle(KeyButtonEvent event) {
		...
		switch (kb.getKeyCode()) {
			case 1000:
				...
				File file = new File("c:\\example.docx");
				try {
...
//			setKeyboardType(ctrlProperty.get(), !shiftProperty.get(), symbolProperty.get());
				break;
			case 1001:
				System.out.println("Start Leap Motion Visualiser");

			try {
				Runtime.getRuntime().exec("C:\\Program Files (x86)\\Leap Motion\\Core Services\\VisualizerApp.exe");
			} catch (IOException e) {
				e.printStackTrace();
			}
				break;
			case 1002:
                System.out.println("Leap Motion Listener Running");
                controller.addListener(mouse);

				break;
			case 1003:
                System.out.println("Leap Motion Listener Stopped");
